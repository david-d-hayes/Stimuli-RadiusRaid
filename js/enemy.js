...
$.Enemy.prototype.receiveDamage = function( i, val ) {
	...
	if( this.life <= 0 ) {
		...
		// Add enemies defeated variable
		$.enemiesDefeated++;

		// Track enemy type defeated
		$.enemyDefeatCount[this.type]++;
...
$.Enemy.prototype.renderPreview = function(ctx, x, y, scale) {
    var r = this.radius * scale;
    var mod = $.enemyOffsetMod / 6;

    // Base fill
    $.util.fillCircle(ctx, x, y, r, this.fillStyle);

    // Inner animated stroke
    $.util.strokeCircle(ctx, x, y, r / 4 + Math.cos(mod) * r / 4, this.strokeStyle, 1.5);

    // Outer stroke
    $.util.strokeCircle(ctx, x, y, r - 0.5, this.strokeStyle, 1);

    // Animated arcs
    ctx.strokeStyle = this.strokeStyle;
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.arc(x, y, r - 0.5, mod + $.pi, mod + $.pi + $.pi / 2);
    ctx.stroke();
    ctx.beginPath();
    ctx.arc(x, y, r - 0.5, mod, mod + $.pi / 2);
    ctx.stroke();
};
